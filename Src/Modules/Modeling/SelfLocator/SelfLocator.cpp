...
void SelfLocator::handleGameStateChanges(const Pose2f& propagatedRobotPose)
{
    ...
    if(theGameInfo.state != STATE_PLAYING)
    {
        ...
        {
          for(int i=0; i<samples->size(); ++i)
            samples->at(i).init(Pose2f(0.f, (theFieldDimensions.xPosOpponentGroundline - 600.f), theFieldDimensions.yPosLeftSideline), *this, nextSampleNumber++);
          sampleSetHasBeenResetted = true;
        }
    }
...
void SelfLocator::draw(const RobotPose& robotPose)
{
  ...
  COMPLEX_DRAWING("module:SelfLocator:fieldOfView") 	//SPQR - was "sample" instead of "fieldOfView"
    for(int i = 0; i < numberOfSamples; ++i) \
      samples->at(i).draw();
