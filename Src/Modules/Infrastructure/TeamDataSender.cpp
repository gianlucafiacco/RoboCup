...
void TeamDataSender::update(TeamDataSenderOutput& teamDataSenderOutput)
{
    if(theTeammateData.sendThisFrame)
    {
        ...
        //SPQR
        TEAM_OUTPUT(idRobotPoseSpqrFiltered, bin, RobotPoseSpqrFilteredCompressed(theRobotPoseSpqrFiltered));
        TEAM_OUTPUT(idSpqrDWKcombiner, bin, SpqrDWKcombinerCompressed(theSpqrDWKcombiner));
        TEAM_OUTPUT(idGlobalBallEstimation, bin, GlobalBallEstimationCompressed(theGlobalBallEstimation));
