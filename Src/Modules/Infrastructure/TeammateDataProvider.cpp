...
#include "../spqr_modules/EQualityNetwork/EQualityNetwork.h"
...
TeammateDataProvider::TeammateDataProvider() : lastSentTimestamp(0), lastReceivedTimestamp(0),
    currentTeammate(0), teammateDataPtr(0), pos_infos_to_add(-1)
{
    theInstance = this;
}
...
void TeammateDataProvider::update(TeammateData& teammateData)
{
    ...
    // EMANUELE ---------------------------------------- TODO Check Vincenzo
    //if( (counter_bufferCoordination++ % 5) == 0 ) // 0 & 5. If not added pos not increased, so thisi is still true on the next iteration
    //{
    int temp = (((int)pos_infos_to_add) + 1) % 10;
    if(teammateData.coordinationBuffer.add(
                SystemCall::getCurrentSystemTime(),
                theRobotInfo.number,
                (float)(theRobotPose.validity),
                (float)(theRobotPoseSpqrFiltered.x),
                (float)(theRobotPoseSpqrFiltered.y),
                temp,
                true
                )
            )
    {
        pos_infos_to_add = (((int)pos_infos_to_add) + 1) % 10;
    }
    //}
...
bool TeammateDataProvider::handleMessage(InMessage& message)
{
    ...
    case idTeammateRoles:
        ...
        // SPQR
    case idGlobalBallEstimation:
        if(currentTeammate)
            message.bin >> currentTeammate->globalBallEstimation;
        return true;

    case idRobotPoseSpqrFiltered:
        if(currentTeammate)
        {
            message.bin >> currentTeammate->robotPoseSpqrFiltered;

            teammateDataPtr->coordinationBuffer.add(
#ifdef TARGET_SIM
                        SystemCall::getCurrentSystemTime(),
#else
                        lastReceivedTimestamp, //TODO Check Vincenzo
#endif
                        currentTeammate->number, currentTeammate->pose.validity,
                        currentTeammate->robotPoseSpqrFiltered.x,
                        currentTeammate->robotPoseSpqrFiltered.y, -1);
        }
        return true;

    case idSpqrDWKcombiner:
        if(currentTeammate)
            message.bin >> currentTeammate-> spqrDWKcombination;
        return true;

    case idEQualityNTPRequest:
    {
        unsigned time = SystemCall::getCurrentSystemTime();
        EQualityNetwork::EQualityRequest request;
        message.bin >> request;
        EQualityNetwork::replyToMessage( request, time, (unsigned char)theRobotInfo.number );
        //      std::cerr << "RECEIVED A REQUEST" << std::endl;
        return true;
    }
    case idEQualityNTPResponse:
    {
        EQualityNetwork::EQualityResponse response;
        message.bin >> response;
        response.receipt = SystemCall::getCurrentSystemTime();
        if( (int)response.reply_to == theRobotInfo.number )
        {
            //          std::cerr << "[" << theRobotInfo.number << "] : Origine -> " << (unsigned)response.origin << " RTT -> " << (int)response.compute().roundTrip << std::endl;
            EQualityNetwork::computeQualities(theRobotInfo.number, (int)response.sender, (int)response.compute().roundTrip, (int)response.compute().offset);
        }
        return true;
    }
