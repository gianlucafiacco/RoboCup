...
#ifdef AUTO_BALANCE
        if(DynamicCameraCompensation::enabled && SystemCall::getTimeSince(prev_timeStamp) > BALANCE_TIME){
            DynamicCameraCompensation::switchCameraSettings(upperCameraSettings, lowerCameraSettings);
            prev_timeStamp = SystemCall::getCurrentSystemTime();
        }
        else if(!DynamicCameraCompensation::enabled && SystemCall::getTimeSince(prev_timeStamp) > CHECK_TIME) {
            DynamicCameraCompensation::checkGrid(image, theColorTable, std::max(2, static_cast<int>(theImageCoordinateSystem.origin.y())), upperCameraSettings, lowerCameraSettings);
            prev_timeStamp = SystemCall::getCurrentSystemTime();
        }
#endif
